...
namespace TygaSoft.WcfService
{
    public partial class CollectLifeService
    {
        ...
        /// <param name="cmdParms"></param>
        /// <returns></returns>
        public string GetResidenceCommunityList(int pageIndex, int pageSize, string sqlWhere, params SqlParameter[] cmdParms)
        {
            ResidenceCommunity bll = new ResidenceCommunity();
            List<ResidenceCommunityInfo> list = bll.GetList(pageIndex, pageSize, sqlWhere, cmdParms);
            if (list == null || list.Count == 0) return "[]";
            return JsonConvert.SerializeObject(list);
        }
