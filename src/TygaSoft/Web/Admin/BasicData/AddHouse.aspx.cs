    ...
    public partial class AddHouse : System.Web.UI.Page
    {
        ...
        private void Bind(ref string myDataAppend)
        {
            if (!Id.Equals(Guid.Empty))
            {
                Page.Title = "编辑楼单元信息";
                House bll = new House();
                HouseInfo model = bll.GetModelByJoin(Id);
                if (model != null)
                {
                    txtName.Value = model.HouseCode;
                    txtaRemark.Value = model.Remark;
                    myDataAppend += "{ \"Id\":\"" + model.Id + "\",\"PropertyCompanyId\":\"" + model.PropertyCompanyId + "\",\"ResidenceCommunityId\":\"" + model.ResidenceCommunityId + "\",\"ResidentialBuildingId\":\"" + model.ResidentialBuildingId + "\",\"ResidentialUnitId\":\"" + model.ResidentialUnitId + "\",\"CompanyName\":\"" + model.CompanyName + "\",\"CommunityName\":\"" + model.CommunityName + "\",\"BuildingCode\":\"" + model.BuildingCode + "\",\"UnitCode\":\"" + model.UnitCode + "\"}";
                }
            }
        }
    }
