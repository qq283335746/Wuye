...
var ListResidentialBuilding = {
    Init: function () {
        ListResidentialBuilding.Grid(sPageIndex, sPageSize);
    },
    ...
    Add: function () {
        window.location = "tta.html";
    },
    Edit: function () {
        var cbl = $('#dgT').datagrid("getSelections");
        if (cbl && cbl.length == 1) {
            window.location = "tta.html?Id=" + cbl[0].f0 + "";
        }
        else {
    ...
    Del: function () {
        try {
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/House/ScriptServices/AdminService.asmx/DelResidentialBuilding",
                        type: "post",
