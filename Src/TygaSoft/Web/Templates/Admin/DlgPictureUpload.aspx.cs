    ...
    public partial class DlgPictureUpload : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    myDataAppend.Replace("{CallBack}", string.Format("$(\"#{0}\").dialog('refresh','{1}');", dlgParentId, "/House/t/tpicture.html?dlgId=" + dlgParentId + "&funName=" + funName + "&isMutil=" + isMutilSelect + ""));
                }
                else
                {
