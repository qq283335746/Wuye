...
var ComplainRepair = {
    ...
    Add: function () {
        window.location = "tya.html";
    },
    Edit: function () {
        var cbl = $('#dgT').datagrid("getSelections");
        if (cbl && cbl.length == 1) {
            window.location = "tya.html?Id=" + cbl[0].f0 + "";
        }
        else {
    ...
    Del: function () {
        try {
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/House/ScriptServices/AdminService.asmx/DelComplainRepair",
                        type: "post",
    ...
    StatusFormatter: function (value, row, index) {
        ...
        if (value == 0) {
            return "<a href=\"javascript:void(0)\" class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-remove',plain:true\" onclick=\"ComplainRepair.Deal()\">受理中</a>"
        }
    },
    Deal: function () {
        var rows = $('#dgT').datagrid("getSelections");
        if (!rows || rows.length == 0) {
            $.messager.alert('错误提醒', '请选择一行且仅一行进行处理', 'error');
            return false;
        }
        ...
        for (var i = 0; i < rows.length; i++) {
            itemAppend += rows[i].f0 + ",";
            statusAppend += rows[i].Status + ",";
        }

        $.messager.confirm('温馨提醒', '确定要将改行数据置于”已受理“吗？', function (r) {
            if (r) {
                $.ajax({
                    url: "/House/ScriptServices/AdminService.asmx/DealStatusBatch",
                    type: "post",
                    contentType: "application/json; charset=utf-8",
                    data: '{itemAppend:"' + itemAppend + '",statusAppend:"' + statusAppend + '"}',
                    beforeSend: function () {
                        $("#dlgWaiting").dialog('open');
                    },
                    complete: function () {
