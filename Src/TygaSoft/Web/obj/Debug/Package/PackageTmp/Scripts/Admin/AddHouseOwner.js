...
var AddHouseOwner = {
    ...
    SetForm: function () {
        $.map(AddHouseOwner.GetMyData("myDataForModelInfo"), function (item) {
            $("#lbtnSelect").find(".l-btn-text").text(item.CompanyName + "," + item.CommunityName + "," + item.BuildingCode + "," + item.UnitCode + "," + item.HouseCode);
    ...
    OnDlgOpen: function () {
        ...
        if (!$('#tabParent').tabs('exists', '小区')) {
            ...
            AddHouseOwner.CreateDgCommunity();
        }
        if (!$('#tabParent').tabs('exists', '楼')) {
            ...
            AddHouseOwner.CreateDgBuilding();
        }
        if (!$('#tabParent').tabs('exists', '单元')) {
            ...
            AddHouseOwner.CreateDgUnit();
        }
        if (!$('#tabParent').tabs('exists', '房间')) {
            $('#tabParent').tabs('add', {
                title: '房间',
                selected: false,
                style: { paddingTop: 10 },
    ...
    OnDlgSelect: function () {
        ...
        if (!row) {
            $.messager.alert('错误提示', '请选择物业公司，小区，楼，单元，房间', 'error');
            return false;
        }
        ...
        var rowHouse = $("#dgHouse").datagrid('getSelected');
        if (!rowHouse) {
            $.messager.alert('错误提示', '请选择物业公司，小区，楼，单元，房间', 'error');
            return false;
        }
        $("#lbtnSelect").find(".l-btn-text").text(row.CompanyName + "," + rowCommunity.CommunityName + "," + rowBuilding.BuildingCode + "," + rowUnit.UnitCode + "," + rowHouse.HouseCode);
    ...
    OnSave: function () {
        try {
            ...
            $.ajax({
                url: "/House/ScriptServices/AdminService.asmx/SaveHouseOwner",
                type: "post",
                data: '{model:{Id:"' + Id + '",HouseOwnerName:"' + $.trim($("#txtName").val()) + '",MobilePhone:"' + $.trim($("#txtMobilePhone").val()) + '",TelPhone:"' + $.trim($("#txtTelPhone").val()) + '",PropertyCompanyId:"' + propertyCompanyId + '",ResidenceCommunityId:"' + residenceCommunityId + '",ResidentialBuildingId:"' + residentialBuildingId + '",ResidentialUnitId:"' + residentialUnitId + '",HouseId:"' + houseId + '"}}',
