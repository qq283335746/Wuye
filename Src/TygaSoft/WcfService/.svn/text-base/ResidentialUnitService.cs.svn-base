...
namespace TygaSoft.WcfService
{
    public partial class CollectLifeService
    {
        ...
        /// <param name="cmdParms"></param>
        /// <returns></returns>
        public string GetResidentialUnitList(int pageIndex, int pageSize, string sqlWhere, params SqlParameter[] cmdParms)
        {
            ResidentialUnit bll = new ResidentialUnit();
            List<ResidentialUnitInfo> list = bll.GetList(pageIndex, pageSize, sqlWhere, cmdParms);
            if (list == null || list.Count == 0) return "[]";
            return JsonConvert.SerializeObject(list);
        }
